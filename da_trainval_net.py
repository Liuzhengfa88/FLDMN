...
if __name__ == "__main__":
    ...
    if args.dataset == "pascal_voc":
        ...
        #fake target & fake source
        args.ft_imdb_name = "cityscape_2007_train_ft"
        args.fs_imdb_name = "cityscape_2007_train_fs"
    ...
    #     ]
        ...
        print("loading our dataset...........")
        args.s_imdb_name = "voc_2007_train_s"
        args.t_imdb_name = "clipart_2007_train_t"
        #fake target & fake source
        args.ft_imdb_name = "voc_2007_train_ft"
        args.fs_imdb_name = "clipart_2007_train_fs"

        args.t_imdbtest_name = "clipart_2007_test_t"
        args.set_cfgs = [
            "ANCHOR_SCALES",
            "[8,16,32]",
            "ANCHOR_RATIOS",
        ...
        print("loading our dataset...........")
        args.s_imdb_name = "sim10k_2007_train_s"
        args.t_imdb_name = "cityscape_car_2007_train_t"
        #fake target & fake source
        args.ft_imdb_name = "sim10k_2007_train_ft"
        args.fs_imdb_name = "cityscape_car_2007_train_fs"

        args.s_imdbtest_name = "sim10k_2007_test_s"
    ...
    args.cfg_file = (
        "./FLDMN/cfgs/{}_ls.yml".format(args.net)
        if args.large_scale
        else "./FLDMN/cfgs/{}.yml".format(args.net)
    )
   ...
   # train set
    ...
    #fake_target datasets, zf
    ft_imdb, ft_roidb, ft_ratio_list, ft_ratio_index = combined_roidb(args.ft_imdb_name)
    ft_train_size = len(ft_roidb)  # add flipped         image_index*2
    ...
    #fake_source datasets, zf
    fs_imdb, fs_roidb, fs_ratio_list, fs_ratio_index = combined_roidb(args.fs_imdb_name)
    fs_train_size = len(fs_roidb)  # add flipped         image_index*2
    ...
    #fake_source & fake_target, zf
    # ft_sampler_batch = s_sampler_batch
    # fs_sampler_batch = t_sampler_batch
    ...
    # fs_dataloader = torch.utils.data.DataLoader(
    #     fs_dataset,
    #     batch_size=args.batch_size,
    #     sampler=fs_sampler_batch,
    #     num_workers=args.num_workers,
    # )
    ...
    #fake_source & fake_target, zf
    ft_im_data = torch.FloatTensor(1)
    ft_im_info = torch.FloatTensor(1)
    ft_im_cls_lb = torch.FloatTensor(1)
    ft_num_boxes = torch.LongTensor(1)
    ft_gt_boxes = torch.FloatTensor(1)
    ft_need_backprop = torch.FloatTensor(1)

    fs_im_data = torch.FloatTensor(1)
    fs_im_info = torch.FloatTensor(1)
    fs_im_cls_lb = torch.FloatTensor(1)
    fs_num_boxes = torch.LongTensor(1)
    fs_gt_boxes = torch.FloatTensor(1)
    fs_need_backprop = torch.FloatTensor(1)
    ...
    if args.cuda:
        ...
        ft_im_data = ft_im_data.cuda()
        ft_im_info = ft_im_info.cuda()
        ft_im_cls_lb = ft_im_cls_lb.cuda()
        ft_num_boxes = ft_num_boxes.cuda()
        ft_gt_boxes = ft_gt_boxes.cuda()
        ft_need_backprop = ft_need_backprop.cuda()

        fs_im_data = fs_im_data.cuda()
        fs_im_info = fs_im_info.cuda()
        fs_im_cls_lb = fs_im_cls_lb.cuda()
        fs_num_boxes = fs_num_boxes.cuda()
        fs_gt_boxes = fs_gt_boxes.cuda()
        fs_need_backprop = fs_need_backprop.cuda()
    ...
    #fake_source & fake_target
    ft_im_data = Variable(ft_im_data)
    ft_im_info = Variable(ft_im_info)
    ft_im_cls_lb = Variable(ft_im_cls_lb)
    ft_num_boxes = Variable(ft_num_boxes)
    ft_gt_boxes = Variable(ft_gt_boxes)
    ft_need_backprop = Variable(ft_need_backprop)

    fs_im_data = Variable(fs_im_data)
    fs_im_info = Variable(fs_im_info)
    fs_im_cls_lb = Variable(fs_im_cls_lb)
    fs_num_boxes = Variable(fs_num_boxes)
    fs_gt_boxes = Variable(fs_gt_boxes)
    fs_need_backprop = Variable(fs_need_backprop)
    ...
    if args.cuda:
        fasterRCNN.cuda()
        # fasterRCNN = nn.DataParallel(fasterRCNN, device_ids=[0,3])
    ...
    #fake_source and fake_target
    # ft_data_iter = iter(ft_dataloader)
    # fs_data_iter = iter(fs_dataloader)
    
    loss_temp = 0

    iters_per_epoch = int(30000 / args.batch_size)
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        #fake_source and fake_target,zf
        # data_iter_ft = iter(ft_dataloader)
        # data_iter_fs = iter(fs_dataloader)

        for step in range(iters_per_epoch):
            try:
                data, ft_data = next(data_iter_s)
            except:
                data_iter_s = iter(s_dataloader)
                data, ft_data = next(data_iter_s)
            try:
                tgt_data, fs_data = next(data_iter_t)
            except:
                data_iter_t = iter(t_dataloader)
                tgt_data, fs_data = next(data_iter_t)
            ...
            #fake_source and fake_target, zf
            # try:
            #     ft_data = next(data_iter_ft)
            # except:
            #     data_iter_ft = iter(ft_dataloader)
            #     ft_data = next(data_iter_ft)
            # try:
            #     fs_data = next(data_iter_fs)
            # except:
            #     data_iter_fs = iter(fs_dataloader)
            #     fs_data = next(data_iter_fs)
            ...
            tgt_im_cls_lb.data.resize_(tgt_data[2].size()).copy_(tgt_data[2])
            ...
            #fake_source & fake_target, zf
            ft_im_data.data.resize_(ft_data[0].size()).copy_(ft_data[0])  # change holder size
            ft_im_info.data.resize_(ft_data[1].size()).copy_(ft_data[1])
            ft_im_cls_lb.data.resize_(ft_data[2].size()).copy_(ft_data[2])
            ft_gt_boxes.data.resize_(ft_data[3].size()).copy_(ft_data[3])
            ft_num_boxes.data.resize_(ft_data[4].size()).copy_(ft_data[4])
            ft_need_backprop.data.resize_(ft_data[5].size()).copy_(ft_data[5])

            fs_im_data.data.resize_(fs_data[0].size()).copy_(
                fs_data[0]
            )  # change holder size
            fs_im_info.data.resize_(fs_data[1].size()).copy_(fs_data[1])
            fs_im_cls_lb.data.resize_(fs_data[2].size()).copy_(fs_data[2])
            fs_gt_boxes.data.resize_(fs_data[3].size()).copy_(fs_data[3])
            fs_num_boxes.data.resize_(fs_data[4].size()).copy_(fs_data[4])
            fs_need_backprop.data.resize_(fs_data[5].size()).copy_(fs_data[5])
            ...
            loss = (
                img_cls_loss.mean()
                + tgt_img_cls_loss.mean()
                + contra_src_loss.mean()
                + contra_tgt_loss.mean()
                + rpn_loss_cls.mean()
                + rpn_loss_box.mean()
                + RCNN_loss_cls.mean()
                ...
                + RCNN_loss_bbox.mean()
                + invar_kdl_loss.mean() 
                + tgt_invar_kdl_loss.mean() 
                + args.lamda
                * (
                    DA_img_loss_cls.mean()
                    + local_DA_img_loss_cls.mean()
                    + DA_ins_loss_cls.mean()
                    + tgt_DA_img_loss_cls.mean()
                    +tgt_local_DA_img_loss_cls.mean()
            ...
            # if epoch > 1:
            #     loss = loss + tgt_img_cls_loss.mean()
            ...
            if step % args.disp_interval == 0:
                ...
                loss_category_cls = img_cls_loss.item() +  tgt_img_cls_loss.item()
                loss_contra_src_cls = contra_src_loss.item()
                loss_contra_tgt_cls = contra_tgt_loss.item()

                loss_rpn_cls = rpn_loss_cls.item()
                loss_rpn_box = rpn_loss_box.item()
                # loss_rcnn_cls = RCNN_loss_cls.item() + tgt_RCNN_loss_cls.item()
                loss_rcnn_cls = RCNN_loss_cls.item() 
                loss_rcnn_box = RCNN_loss_bbox.item()

                loss_ins_kdl_cls = invar_kdl_loss.item()
                loss_tgt_ins_kdl_cls = tgt_invar_kdl_loss.item()

                loss_DA_img_cls = (
                    args.lamda
                    * (DA_img_loss_cls.item() + tgt_DA_img_loss_cls.item()
                    + local_DA_img_loss_cls.item() + tgt_local_DA_img_loss_cls.item())
                    / 2
                )
                loss_DA_ins_cls = (
                    args.lamda
                ...
                print(
                    "\t\t\tcategory_cls: %.4f, contra_src_cls: %.4f, contra_tgt_cls: %.4f, rpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box %.4f, ins_kdl %.4f, tgt_ins_kdl %.4f\n\t\t\timg_loss %.4f,ins_loss %.4f,cst_loss %.4f"
                    % (
                        loss_category_cls,
                        loss_contra_src_cls,
                        loss_contra_tgt_cls,
